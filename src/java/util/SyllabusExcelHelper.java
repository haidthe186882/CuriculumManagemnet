...
/**
 * Parse Syllabus Excel file (SWT301 format). Scans sheets for key sections:
 * basic info, CLOs, Sessions, Materials.
 */
public class SyllabusExcelHelper {
    ...
    public static class SyllabusImportData {
        ...
        public void setClos(List<CourseLearningOutcome> clos) {
            this.clos = clos;
        }
        ...
        public List<SyllabusMaterial> getMaterials() {
            return materials;
        }

        public void setMaterials(List<SyllabusMaterial> materials) {
    ...
    private static void parseBasicInfo(Sheet sheet, SyllabusImportData data) {
        ...
        for (int i = 0; i <= sheet.getLastRowNum(); i++) {
            ...
            if (label.contains("min") && label.contains("pass")) {
                ...
                try {
                    s.setMinAvgMarkToPass(Double.parseDouble(val));
        ...
        for (int sheetIdx = 0; sheetIdx < workbook.getNumberOfSheets(); sheetIdx++) {
            ...
            for (int i = 0; i <= sheet.getLastRowNum(); i++) {
                ...
                for (int c = 0; c < row.getLastCellNum(); c++) {
                    ...
                    if (cv.contains("topic") || cv.contains("chu de")) {
                        topicCol = c;
                    }
                    if (cv.contains("learning") && cv.contains("teaching") || cv.contains("type")) {
                        typeCol = c;
                    }
                    if (cv.equals("lo") || cv.contains("learning outcome")) {
                        loCol = c;
                    }
                    if (cv.contains("itu") || cv.contains("i/t/u")) {
                        ituCol = c;
                    }
                    if (cv.contains("student material") || cv.contains("material")) {
                        matCol = c;
                    }
                    if (cv.contains("student task") || cv.contains("task")) {
                        taskCol = c;
                    }
                    if (cv.contains("url") || cv.contains("link")) {
                        urlCol = c;
                    }
                }
                ...
                if (isSessionHeader && topicCol >= 0) {
                    ...
                    for (int j = i + 1; j <= sheet.getLastRowNum(); j++) {
                        ...
                        if (sessionNoStr.isEmpty() && topic.isEmpty()) {
                            break;
                        }
                        ...
                        if (typeCol >= 0) {
                            session.setLearningTeachingType(cellStr(dataRow, typeCol).trim());
                        }
                        if (loCol >= 0) {
                            session.setLo(cellStr(dataRow, loCol).trim());
                        }
                        if (ituCol >= 0) {
                            session.setItu(cellStr(dataRow, ituCol).trim());
                        }
                        if (matCol >= 0) {
                            session.setStudentMaterials(cellStr(dataRow, matCol).trim());
                        }
                        if (taskCol >= 0) {
                            session.setStudentTasks(cellStr(dataRow, taskCol).trim());
                        }
                        if (urlCol >= 0) {
                            session.setUrls(cellStr(dataRow, urlCol).trim());
                        }
    ...
    private static void parseMaterials(Workbook workbook, SyllabusImportData data) {
        for (int sheetIdx = 0; sheetIdx < workbook.getNumberOfSheets(); sheetIdx++) {
            ...
            for (int i = 0; i <= sheet.getLastRowNum(); i++) {
                ...
                for (int c = 0; c < row.getLastCellNum(); c++) {
                    String cv = cellStr(row, c).trim().toLowerCase();
                    if (cv.equals("description") || cv.equals("mô tả") || (cv.contains("material") && (cv.contains("description") || cv.contains("name")))) {
                        descCol = c;
                    }
                    if (cv.equals("author") || cv.contains("tác giả")) {
                        authorCol = c;
                    }
                    if (cv.equals("publisher") || cv.contains("nhà xuất bản")) {
                        pubCol = c;
                    }
                    if (cv.equals("publisheddate") || cv.equals("published date") || cv.contains("published") || (cv.contains("date") && cv.contains("publish"))) {
                        dateCol = c;
                    }
                    if (cv.equals("edition") || cv.contains("ấn bản")) {
                        editionCol = c;
                    }
                    if (cv.equals("isbn")) {
                        isbnCol = c;
                    }
                    if (cv.equals("ismainmaterial") || cv.equals("is main material") || cv.contains("main")) {
                        mainCol = c;
                    }
                    if (cv.equals("ishardcopy") || cv.equals("is hard copy") || cv.contains("hard")) {
                        hardCol = c;
                    }
                    if (cv.equals("isonline") || cv.equals("is online") || cv.contains("online")) {
                        onlineCol = c;
                    }
                    if (cv.equals("link") || cv.contains("url")) {
                        linkCol = c;
                    }
                    if (cv.equals("note") || cv.equals("notes") || cv.contains("ghi chú")) {
                        notesCol = c;
                    }
                }
                ...
                if (isMaterialHeader) {
                    for (int j = i + 1; j <= sheet.getLastRowNum(); j++) {
                        ...
                        if (authorCol >= 0) {
                            m.setAuthor(cellStr(dataRow, authorCol).trim());
                        }
                        if (pubCol >= 0) {
                            m.setPublisher(cellStr(dataRow, pubCol).trim());
                        }
                        if (dateCol >= 0) {
                            try {
                        ...
                        if (editionCol >= 0) {
                            m.setEdition(cellStr(dataRow, editionCol).trim());
                        }
                        if (isbnCol >= 0) {
                            m.setIsbn(cellStr(dataRow, isbnCol).trim());
                        }
                        if (mainCol >= 0) {
                            m.setMainMaterial(isTruthy(cellStr(dataRow, mainCol)));
                        }
                        if (hardCol >= 0) {
                            m.setHardCopy(isTruthy(cellStr(dataRow, hardCol)));
                        }
                        if (onlineCol >= 0) {
                            m.setOnline(isTruthy(cellStr(dataRow, onlineCol)));
                        }
                        if (linkCol >= 0) {
                            m.setLink(cellStr(dataRow, linkCol).trim());
                        }
                        if (notesCol >= 0) {
                            m.setNotes(cellStr(dataRow, notesCol).trim());
                        }
