...
import model.SyllabusCurriculumAlignment;
...
public class SyllabusServlet extends HttpServlet {
    ...
    private void showDetail(HttpServletRequest req, HttpServletResponse res)
        ...
        req.setAttribute("curriculumAlignments", buildCurriculumAlignments(s));
        List<Review> reviews = reviewDAO.getReviewsBySyllabus(id);
        Review latestReview = reviews.isEmpty() ? null : reviews.get(0);
        boolean assignedReviewer = user != null && syllabusDAO.isUserAssignedAsReviewer(id, user.getUserId());
        boolean assignedDesigner = user != null && syllabusDAO.isUserAssignedAsDesigner(id, user.getUserId());
        boolean adminRole = user != null && hasRole(req, "Admin");
        req.setAttribute("reviews", reviews);
        req.setAttribute("latestReview", latestReview);
        req.setAttribute("reviewRubric", SyllabusReviewRubric.getCriteria());
        req.setAttribute("rubricMaximumScore", SyllabusReviewRubric.getMaximumScore());
        req.setAttribute("canReviewSyllabus", user != null && (adminRole || assignedReviewer)
            && SyllabusDAO.STATUS_PENDING_REVIEW.equalsIgnoreCase(s.getStatus()));
        req.setAttribute("canPublishSyllabus", adminRole
            && SyllabusDAO.STATUS_APPROVED_FOR_PUBLISH.equalsIgnoreCase(s.getStatus()));
        req.setAttribute("canEditSyllabus", user != null && (adminRole || assignedDesigner || hasRole(req, "Designer", "Lecturer")));
    ...
    private void showEdit(HttpServletRequest req, HttpServletResponse res)
        ...
        if (syllabus != null) {
            req.setAttribute("clos", cloDAO.getCLOsBySyllabus(syllabus.getSyllabusId()));
            req.setAttribute("sessions", sessionDAO.getSessionsBySyllabus(syllabus.getSyllabusId()));
            req.setAttribute("materials", syllabusDAO.getMaterialsBySyllabusId(syllabus.getSyllabusId()));
        }
        req.getRequestDispatcher("/WEB-INF/views/syllabus/form.jsp").forward(req, res);
    }
